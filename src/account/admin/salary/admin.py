...
class EmployeeSalaryInline(admin.TabularInline):
    ...
    readonly_fields = (
        ...
        "formatted_food_allowance",
        "formatted_festival_bonus",
        "formatted_leave_bonus",
        "get_tax_loan",
        "get_salary_loan",
    ...
    def get_exclude(self, request, obj=None):
        ...
        if not request.user.is_superuser and not request.user.has_perm(
            "account.can_see_salary_on_salary_sheet"
        ):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if not request.user.is_superuser and not request.user.has_perm(
            ...
            readonly_fields = [
                ...
                if field not in self.superadminonly_fields
            ]
    ...
    def get_details(self, obj, *args, **kwargs):
        return format_html(
            f'<a href="/media/temp_emp_salary/{obj.employee.id}.txt" download>Download</a>'
        )
...
class SalarySheetAdmin(SalarySheetAction, admin.ModelAdmin):
    ...
    def save_model(self, request, salary_sheet, form, change):
        ...
        if (
            "festival_bonus" in request.POST
            and request.POST["festival_bonus"] == "on"
        ):
    ...
    def get_list_display(self, request):
        ...
        if (
            ...
            and "total" in list_display
        ):
            print(
                request.user,
                request.user.has_perm("account.can_see_salary_on_salary_sheet"),
            )
            list_display.remove("total")
        return tuple(list_display)

    def total(self, obj):
        total_value = EmployeeSalary.objects.filter(
            salary_sheet_id=obj.id
...
class SalaryReportAdmin(admin.ModelAdmin):
    ...
    def export_tds_pdf_for_inactive_employee(self, request, queryset):
        ...
        pdf.context = {
            "employees": self.get_employee_salary_with_challan(obj, False),
            "obj": obj,
            "tds_challan": tds_challan,
        }
        return pdf.render_to_pdf(download=True)
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        for employee in employees:
            ...
            total_late_fine = (
                ...
                ).aggregate(late_fine=Sum("total_late_attendance_fine"))[
                    "late_fine"
                ]
                or 0
            )
    ...
    def export_as_excel(self, request, queryset):
        if queryset.count() != 1:
            self.message_user(
                request,
                "Please select exactly one Salary Report.",
                level="error",
            )
            return
        ...
        response["Content-Disposition"] = (
            f"attachment; filename=Salary_Report_{start_date}_to_{end_date}.xlsx"
        )
        wb.save(response)
        return response
