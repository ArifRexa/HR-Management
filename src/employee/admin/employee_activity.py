...
class TrialEmployeeAttendanceAdmin(admin.ModelAdmin):
    ...
    def custom_changelist_view(self, request, *args, **kwargs) -> TemplateResponse:
        ...
        paginator = Paginator(list(date_datas.items()), 10)  # Show 10 items per page

        try:
            date_datas_paginated = paginator.page(page_number)
        except PageNotAnInteger:
            date_datas_paginated = paginator.page(1)
        except EmptyPage:
            date_datas_paginated = paginator.page(paginator.num_pages)

        context = dict(
            self.admin_site.each_context(request),
            dates=last_x_dates,
            last_month=last_month,
            date_datas=date_datas_paginated,  # Use paginated data
            o=o,  # order key
            online_status_form=online_status_form,
            paginator=paginator,  # Pass paginator to the context if needed
        )
        return TemplateResponse(
            request, "admin/employee/employee_attendance.html", context
        )
