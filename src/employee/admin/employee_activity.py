...
class TrialEmployeeAttendanceAdmin(admin.ModelAdmin):
    ...
    def custom_changelist_view(self, request, *args, **kwargs) -> TemplateResponse:
        ...
        # Fetch employees with their last month's attendance and daily hours
        emps = (
            ...
            .annotate(
                ...
                total_hours=Coalesce(
                    Sum(
                        ...
                        filter=Q(
                            dailyprojectupdate_employee__created_at__date__gte=last_x_date,
                            dailyprojectupdate_employee__status="approved",
                        ),
                    ),
                    0,
                    output_field=FloatField(),  # Specify output_field to avoid mixed types error
                ),
            )
            .order_by("full_name")
        )
        ...
        for emp in emps.prefetch_related(
            "employeeattendance_set__employeeactivity_set"
        ):
            temp = {}
            attendances = emp.employeeattendance_set.all()
            ...
            for date in last_x_dates:
                ...
                # Calculate accepted hours and attendance data
                for attendance in attendances:
                    if attendance.date == date:
                        activities = attendance.employeeactivity_set.all()
                        if activities.exists():
    ...
    def check_if_late(self, start_time, emp):
        if start_time:
            late_time_change_date = datetime.datetime.strptime(
                "2025-02-11", "%Y-%m-%d"
            ).date()
            ...
            if today >= late_time_change_date:
                return (
                    ...
                    and (
                        start_time.hour >= 12
                        or (start_time.hour == 11 and start_time.minute > 10)
                    )
                ) or (
                    not is_lead
                    and (
                ...
                return (
                    ...
                    and (
                        start_time.hour >= 12
                        or (start_time.hour == 11 and start_time.minute > 30)
                    )
                ) or (
                    not is_lead
                    and (
    ...
    def generate_monthly_attendance_report_xlsx(self, request, *args, **kwargs):
        ...
        for column in attendance_sheet.columns:
            ...
            attendance_sheet.column_dimensions[column].width = adjusted_width

        response = save_virtual_workbook(wb)
        response["Content-Disposition"] = (
            f'attachment; filename="employee_attendance_report.xlsx"'
        )
        return response
