...
class TPMObj:
    ...
    def _get_employee_hours(self) -> Manager[EmployeeProjectHour]:
        if self.__cached_employee_hours_qs is None:
            start_date, end_date = self._get_date_range_by_week(-1)
    ...
    def _get_date_range_by_week(self, week=0, days=7):
        """
        week: int
            1 (next week)
            0 (current week)
    ...
    def _get_hours_count(self, start_date: datetime, end_date: datetime) -> float:
        hours = 0.0
        employee_hours = self._get_employee_hours()
        for entry in employee_hours:
            entry_date = entry.project_hour.date
            if entry_date <= end_date and entry_date > start_date:
                hours += entry.hours
        return hours

    @property
    def last_week_hours(self) -> float:
        start_date, end_date = self._get_date_range_by_week(week=-1, days=WORKING_DAYS_IN_A_WEEK)
        hours = self._get_hours_count(
    ...
    @property
    def last_week_hours_2nd(self) -> float:
        start_date, end_date = self._get_date_range_by_week(week=-2, days=WORKING_DAYS_IN_A_WEEK)
        hours = self._get_hours_count(
            start_date=start_date,
            end_date=end_date
        )
    ...
    @property
    def last_week_hours_3rd(self) -> float:
        start_date, end_date = self._get_date_range_by_week(week=-3, days=WORKING_DAYS_IN_A_WEEK)
        hours = self._get_hours_count(
            start_date=start_date,
            end_date=end_date
        )
    ...
    @property
    def last_week_hours_4th(self) -> float:
        start_date, end_date = self._get_date_range_by_week(week=-4, days=WORKING_DAYS_IN_A_WEEK)
        hours = self._get_hours_count(
            start_date=start_date,
            end_date=end_date
        )
